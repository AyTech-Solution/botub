...
import { doc, getDoc, collection, query, where, onSnapshot, addDoc, updateDoc, arrayUnion, getDocs } from 'firebase/firestore';
...
export default function WidgetPage() {
      ...
      try {
        ...
        if (botSnap.exists()) {
          ...
          const unsubscribeKnowledge = onSnapshot(qKnowledge, (snapshot) => {
            ...
            handleFirestoreError(error, OperationType.GET, `bots/${botId}/knowledge`);
          });
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = {
        id: (Date.now() + 1).toString(),
        role: 'bot',
        content: "I'm having trouble connecting right now. Please check your data or try again!",
        timestamp: new Date().toISOString()
      };
  ...
  return (
    <div className="h-screen flex flex-col bg-white overflow-hidden">
      {/* Header */}
      <div 
        className="p-4 border-b border-gray-100 flex items-center justify-between text-white shrink-0 shadow-lg"
        style={{ backgroundColor: bot.branding?.color || '#4f46e5' }}
      >
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-sm font-black leading-tight">{bot.name}</h1>
        ...
        {messages.map((msg) => (
          ...
          >
            <div 
              ...
              style={msg.role === 'user' ? { backgroundColor: bot.branding?.color || '#4f46e5' } : {}}
            >
              <div className="prose prose-sm max-w-none">
                <ReactMarkdown>{msg.content}</ReactMarkdown>
              </div>
              <div className={`text-[10px] mt-1.5 font-bold ${msg.role === 'user' ? 'text-white/70' : 'text-gray-400'}`}>
          ...
          <div className="flex justify-start">
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
              <div className="flex space-x-1">
                <div className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ backgroundColor: bot.branding?.color || '#4f46e5', opacity: 0.4 }} />
                <div className="w-1.5 h-1.5 rounded-full animate-bounce [animation-delay:0.2s]" style={{ backgroundColor: bot.branding?.color || '#4f46e5', opacity: 0.4 }} />
                <div className="w-1.5 h-1.5 rounded-full animate-bounce [animation-delay:0.4s]" style={{ backgroundColor: bot.branding?.color || '#4f46e5', opacity: 0.4 }} />
              </div>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSend} className="flex items-center space-x-2">
          <div className="flex-grow relative">
            <input
              ...
              className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl focus:ring-2 focus:ring-brand-primary/10 transition-all outline-none text-sm font-medium pr-10"
              style={{ borderColor: bot.branding?.color ? `${bot.branding.color}20` : undefined }}
            />
            {bot.voiceEnabled && (
              <button
                type="button"
                ...
                style={{ color: isListening ? '#ef4444' : bot.branding?.color || '#4f46e5' }}
              >
          ...
          <button
            ...
            className="p-3 text-white rounded-xl transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
            style={{ backgroundColor: bot.branding?.color || '#4f46e5' }}
          >
        ...
        {bot.showPoweredBy && (
          <div className="mt-2 text-center">
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center justify-center">
              Powered by <Sparkles className="w-3 h-3 mx-1" style={{ color: bot.branding?.color || '#4f46e5' }} /> Botub
            </p>
          </div>
        )}
      </div>
